...
class LaserPublisherNode(Node):
    ...
    def __init__(self):
        ...
        self.current_frequency = 10.0  
               #nötig für die Vermeidung des Fehlers
        self.scan= []
    ...
    def callback_set_scan_frequency(self,request: SetScanFrequency.Request, response: SetScanFrequency.Response):
        ...
        if angefragte_frequenz in LaserPublisherNode.VALID_SCAN_FREQUENCIES:
           ...
           self.current_frequency=angefragte_frequenz
           ...
           response.message = (f"Scan-Frequenz {angefragte_frequenz} Hz abgelehnt. "f"Gültige Werte in Hz: {LaserPublisherNode.VALID_SCAN_FREQUENCIES}.")
           self.get_logger().warn(f"Anfrage zur Scan-Frequenz: {angefragte_frequenz} Hz abgelehnt")
           #', '.join(str(f) for f in LaserPublisherNode.VALID_SCAN_FREQUENCIES)
        return response
